	...
	void averageModelWeights(float alpha, const Graph &model, float beta, Graph &averaged)
	{
		for (int i = 0; i < model.numberOfNodes(); i++)
		{
			...
			if (not src.getWeights().getParam().isEmpty())
				addTensors(model.context(), alpha, src.getWeights().getParam(), 0.0f, Tensor(), 0.0f, Tensor(), beta, dst.getWeights().getParam());
			if (not src.getBias().getParam().isEmpty())
				addTensors(model.context(), alpha, src.getBias().getParam(), 0.0f, Tensor(), 0.0f, Tensor(), beta, dst.getBias().getParam());
		}
	}
