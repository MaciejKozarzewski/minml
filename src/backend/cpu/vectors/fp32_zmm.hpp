	...
	class Vector<float, ZMM>
	{
		...
		public:
			...
			void load(const sw_float16 *src, int num = size()) noexcept
			{
				assert(0 <= num && num <= size());
				const __m256i tmp = Loader<XMM>()(reinterpret_cast<const uint16_t*>(src), num);
				m_data = Converter<ZMM, sw_float16, float>()(tmp);
			}
			void load(const bfloat16 *src, int num = size()) noexcept
			{
				assert(0 <= num && num <= size());
				const __m256i tmp = Loader<XMM>()(reinterpret_cast<const uint16_t*>(src), num);
				m_data = Converter<ZMM, bfloat16, float>()(tmp);
			}
			void load(const sw_bfloat16 *src, int num = size()) noexcept
			{
				assert(0 <= num && num <= size());
				const __m256i tmp = Loader<XMM>()(reinterpret_cast<const uint16_t*>(src), num);
				m_data = Converter<ZMM, sw_bfloat16, float>()(tmp);
			}
			...
			{
				assert(0 <= num && num <= size());
				const __m256i tmp = Converter<ZMM, float, sw_float16>()(m_data);
				Storer<XMM>()(reinterpret_cast<uint16_t*>(dst), tmp, num);
			}
			void store(bfloat16 *dst, int num = size()) const noexcept
			{
				assert(0 <= num && num <= size());
				const __m256i tmp = Converter<ZMM, float, bfloat16>()(m_data);
				Storer<XMM>()(reinterpret_cast<uint16_t*>(dst), tmp, num);
			}
			void store(sw_bfloat16 *dst, int num = size()) const noexcept
			{
				assert(0 <= num && num <= size());
				const __m256i tmp = Converter<ZMM, float, sw_bfloat16>()(m_data);
				Storer<XMM>()(reinterpret_cast<uint16_t*>(dst), tmp, num);
			}
			template<typename T>
...
//				}
			...
			void cutoff(const int num, Vector<float, ZMM> value = zero()) noexcept
			{
//				switch (num)
//				{
//					case 0:
...
//				}
			...
			{
				return Vector<float, ZMM>(scalar_zero()); // @suppress("Ambiguous problem")
			}
			static Vector<float, ZMM> one() noexcept
			{
				return Vector<float, ZMM>(scalar_one()); // @suppress("Ambiguous problem")
			}
			static Vector<float, ZMM> epsilon() noexcept
			{
				return Vector<float, ZMM>(scalar_epsilon()); // @suppress("Ambiguous problem")
			}
