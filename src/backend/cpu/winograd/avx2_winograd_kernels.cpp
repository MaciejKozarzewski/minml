...
namespace ml
{
	...
	void winograd_input_transform_4x4_3x3_avx2_fma_fp32(const void *src[], void *dst[], void *workspace, int filters)
	{
		...
		add(imm(6*1*4), rbx)// add 6*8 (6*1 floats) to rbx (workspace), moving to next row
		add(imm(6*8), rcx)// add 6*8*4 (6 pointers) to rcx (dst), moving to next row

		dec(r8)
		jne(TRANSFORM2x1)
		sub(imm(6*6*1*4), rbx)// add 6*8 (6*1 floats) to rbx (workspace), moving to next row
		sub(imm(6*6*8), rcx)// add 6*8*4 (6 pointers) to rcx (dst), moving to next row
