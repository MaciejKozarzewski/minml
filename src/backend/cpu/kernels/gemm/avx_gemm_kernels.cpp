...
namespace ml
{
	...
	void gemm_avx_8x8_fp32(int M, int N, int K, const void *alpha_ptr, const void *__restrict__ lhs_ptr, const void *__restrict__ rhs_ptr,
	...
	void pack_avx_8xK_fp32(int M, int K, const void *__restrict__ src, void *__restrict__ dst, char op)
	{
		assert(M == 8);
		assert(K >= 0);
		assert(src != nullptr);
