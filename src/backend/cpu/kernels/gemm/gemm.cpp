...
//		}
	...
	TileDimensions get_outer_tile(mlContext_t context, mlDataType_t dtype) noexcept
	{
		...
		return TileDimensions { 192, 192, 256 };
	}
	...
	class ArrayOfFragments
	{
		...
		public:
			...
			void create(cpu::WorkspaceAllocator &allocator, mlDataType_t dtype, Size2D max_size)
			{
				m_max_size = max_size;
...
//	}
	...
	{
		std::cout << "Printing fragment of size " << frag.rows() << "x" << frag.columns() << '\n';
...
//				std::cout << "pack_d_count = " << pack_d_count << '\n';
...
//				exit(0);
		...
		private:
			...
			{
//				std::cout << "pack_fragment_A(" << &fragment << ", " << m << ", " << k << ")\n";
				const Size2D size_left = transform(total_size.M - m, total_size.K - k, op_A);

//				std::cout << "size left = " << size_left.rows << " " << size_left.columns << '\n';
				const int rows_to_pack = std::min(inner_tile.K, size_left.rows);
				const int cols_to_pack = std::min(inner_tile.M, size_left.columns);
//				std::cout << "packing " << rows_to_pack << " " << cols_to_pack << '\n' << '\n';
				fragment.mark_as_packed_with_size(Size2D(rows_to_pack, cols_to_pack), fragment.stride());
...
//				pack_def_MxK_fp32(fragment, matrix_A, pos, op_A);
			...
			{
//				std::cout << "pack_fragment_B(" << &fragment << ", " << n << ", " << k << ")\n";
				const Size2D size_left = transform(total_size.K - k, total_size.N - n, op_B);

//				std::cout << "size left = " << size_left.rows << " " << size_left.columns << '\n';
				const int rows_to_pack = std::min(inner_tile.K, size_left.rows);
				const int cols_to_pack = std::min(inner_tile.N, size_left.columns);
//				std::cout << "packing " << rows_to_pack << " " << cols_to_pack << '\n' << '\n';
				fragment.mark_as_packed_with_size(Size2D(rows_to_pack, cols_to_pack), fragment.stride());
...
//				pack_def_MxK_fp32(fragment, matrix_B, pos, op_B);
			...
			{
//				std::cout << "pack_fragment_C(" << &fragment << ", " << m << ", " << n << ")\n";
...
//				pack_def_MxK_fp32(fragment, matrix_C, Position2D(m, n), MatrixOp::NORMAL);
			...
			{
//				std::cout << "pack_fragment_D(" << &fragment << ", " << m << ", " << n << ")\n";
				const int rows_to_pack = std::min(inner_tile.M, total_size.M - m);
				const int cols_to_pack = std::min(inner_tile.N, total_size.N - n);
//				std::cout << "packing " << rows_to_pack << " " << cols_to_pack << '\n' << '\n';
...
//				std::cout << "unpacking " << rows_to_unpack << " " << cols_to_unpack << '\n' << '\n';
