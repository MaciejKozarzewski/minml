...
namespace ml
{
	...
	{
		assert(input.size() == 2);
		channelScalingForward(context(), 1.0f, input[0], input[1], 0.0f, output);
	}
	...
	{
		assert(input.size() == 2);
		channelScalingBackward(context(), 1.0f, gradient_next, input[0], input[1], beta[0], gradient_prev[0], beta[1], gradient_prev[1]);
	}
